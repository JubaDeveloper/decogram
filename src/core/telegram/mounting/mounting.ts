...
export const registerBotAndBootstrap = (
    ...
    bot.on("message", async ctx => {
        ...
        await evalListenersForContext(onMessageMetaKey, ctx, messageListeners, sessionContexts, logger);
    });

    bot.on("callback_query", async ctx => {
        logger.trace("[BotEvent] callback_query received");
        await evalListenersForContext(onCallbackMetaKey, ctx, callbackQueryListeners, sessionContexts, logger);
    });
